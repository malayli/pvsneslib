...
void bgInitMapTileSet7LzEx(u8 *tileSource, u8 *mapSource, u8 *tilePalette, u16 address)
{
    setBrightness(0);
    WaitForVBlank();
    dmaCopyVram7(mapSource, address, 0x4000, VRAM_INCLOW | VRAM_ADRTR_0B | VRAM_ADRSTINC_1, 0x1800);
    bgSetMapPtr(0, address, SC_32x32);
    LzssDecodeVram7Ex_src = tileSource;
    LzssDecodeVram7Ex_address = address;
    LzssDecodeVram7Ex();
...
int main(void)
{
    ...
    bgInitMapTileSet7LzEx(&patterns, &map, &palette, 0x0000);
